...
// the other one again.
// Relay drive polarity. The two meters use relay modules that energise on
// opposite logic levels, so this is the one value that genuinely differs
// between the boards. It is selected by the wrapper sketch that includes
// this file -- energitoken_meter_espA or energitoken_meter_espB -- rather
// than edited by hand before each flash. Flashing a board with the wrong
// value inverts every relay decision on it: a commanded "off" energises the
// coil instead, which on a board switching live mains is a hazard.
#if defined(BOARD_ESP_B)
  #define RELAY_CLOSED  HIGH   // ESP-B (F94098): HIGH energises the coil
  #define RELAY_OPEN    LOW
#elif defined(BOARD_ESP_A)
  #define RELAY_CLOSED  LOW    // ESP-A (4BF6F0): LOW energises the coil
  #define RELAY_OPEN    HIGH
#else
  #error "Include this from energitoken_meter_espA or energitoken_meter_espB -- neither BOARD_ESP_A nor BOARD_ESP_B is defined, so the relay polarity is unknown."
#endif
...
// How long to wait before trying to (re)open the override stream after a
// failure. Without this the service block below re-attempts every
// OVSTREAM_CHECK_MS, which is 30ms -- a board that cannot open the stream
// then spends its whole loop failing at it. One boot produced 596 identical
// errors in 25 seconds and published no readings at all in that time.
#define OVSTREAM_RETRY_MS     5000UL
...
// Whether that stream is genuinely established. beginStream() can report
// success before Firebase holds a token, so this records the difference
// between "open" and "open but failing every read".
bool     ovStreamUp  = false;
bool     ovErrLogged = false;   // first error of an episode logged?
uint32_t tOvRetry    = 0;       // last (re)open attempt
...
/* ---------------------------------------------------------------------------
 *  Energy consumed since local midnight, tracked separately from the budget
 *  cycle.
 *
 *  budget.cycleWh answers "how much of the current allowance is spent" and
 *  therefore has to restart whenever a budget is set -- otherwise energy used
 *  before the household had a budget is charged against it. That is correct
 *  for enforcement and wrong for reporting: it means the figure a household
 *  reads as "used today" silently restarts mid-afternoon, and the day's real
 *  total is lost.
 *
 *  This counter answers the other question and resets on one condition only:
 *  the calendar day changing. Setting, changing or clearing a budget does not
 *  touch it.
 * -------------------------------------------------------------------------*/
float todayWh         = 0;      // Wh consumed since local midnight
float todayBaselineWh = 0;      // lifetime reading when the day began
bool  todayBaselineSet = false;
int   todayYday        = -1;    // tm_yday the baseline belongs to

// Defined further down, next to startNewCycle(). Declared here because
// runAlgorithm() calls it above that point, and this file is an included
// header -- it does not get the automatic prototypes the IDE generates for
// a .ino.
 ...
 * -------------------------------------------------------------------------*/
/* ---------------------------------------------------------------------------
 *  Keeps todayWh on the current calendar day and recomputes it.
 *
 *  Called from runAlgorithm() on every valid reading, so it does not depend on
 *  the midnight check firing at any particular moment: a board that boots
 *  after midnight, or whose clock syncs late, corrects itself on the first
 *  reading it takes with a valid clock.
 *
 *  Needs a real wall clock to know what day it is. Without NTP the day cannot
 *  be established, so the baseline is left alone and todayWh simply keeps
 *  accumulating from whenever it was last valid -- wrong, but visibly so, and
 *  preferable to resetting the household's daily figure at an arbitrary moment
 *  because the meter guessed at the date.
 * -------------------------------------------------------------------------*/
void updateTodayEnergy() {
  if (!meas.valid) return;
  ...
  if (now >= 1700000000UL) {
    ...
    if (!todayBaselineSet || tmNow.tm_yday != todayYday) {
      todayBaselineWh  = meas.energy;
      todayBaselineSet = true;
      todayYday        = tmNow.tm_yday;
      saveNVS();
      Serial.printf("[TODAY] new day, baseline=%.0f Wh\n", todayBaselineWh);
    }
  } else if (!todayBaselineSet) {
    // No clock yet, and nothing to measure from. Anchor here so the figure is
    // at least well-defined; the branch above re-anchors once NTP lands.
    todayBaselineWh  = meas.energy;
    todayBaselineSet = true;
  }

  // A negative difference means the module's lifetime counter was reset or
  // replaced under us -- re-anchor rather than publish a negative total.
  float used = meas.energy - todayBaselineWh;
  if (used < 0) {
    todayBaselineWh = meas.energy;
    used = 0;
    saveNVS();
...
void pushState() {
  ...
  // Energy since local midnight. Kept separate from energyWh, which is
  // cycle-scoped and restarts whenever a budget is set.
  if (todayBaselineSet) j.set("energyTodayWh", todayWh);
...
void pullConfig() {
  ...
  // Set by the budgetWh block below when the household's allowance actually
  // changes, and read by the cycleStartedAt block further down. The two
  // arrive in the same write from the app and have to be interpreted
  // together -- see the cycleStartedAt block for why.
  bool budgetChangedThisPull = false;
  ...
  if (json.get(result, "cycleStartedAt") && result.success) {
    ...
    if (v > 0 && v != budget.cycleStartedAt) {
      // Two different writers land here and they mean different things.
      //
      // The scheduled cycle-tick writes this as a heartbeat, and must not be
      // allowed to roll the cycle more than once a day -- that is what the
      // same-day check below is for, and without it a run of frequent firings
      // handed a household extra allowance.
      //
      // The app writes it atomically with budgetWh when the household sets a
      // budget, and that is a deliberate action that must always start a
      // fresh cycle. Treating it as a same-day heartbeat meant energy used
      // BEFORE the budget existed was counted against it: a household that
      // had used 4 Wh and then set a 2 Wh daily allowance was instantly at
      // 200%, which latched applyBudgetGate(), opened every relay including
      // critical, cleared their overrides and refused to accept new ones --
      // a total blackout produced by the act of budgeting, with no way out
      // until midnight.
      if (budget.cycleStartedAt == 0 || unixMillis() == 0
          || budgetChangedThisPull
          || !sameLocalDay(v, budget.cycleStartedAt)) {
        startNewCycle(v, budgetChangedThisPull ? "budget set by household"
 ...
 * ===========================================================================*/
/* ---------------------------------------------------------------------------
 *  Opens the relayOverrides stream, but only once Firebase can actually
 *  authenticate a request.
 *
 *  fbReady alone is not enough: it records that initFirebase() has run, not
 *  that a token has been obtained. Beginning a stream in that window appears
 *  to succeed and then fails on every read with "missing required
 *  credentials", which is what a board coming up on a slow network used to
 *  do -- WiFi associates, the stream opens against no credentials, and the
 *  service block retries it every 30ms from then on.
 *
 *  Returns true only when the stream is genuinely established, so the caller
 *  can tell "open" from "opened but useless".
 * -------------------------------------------------------------------------*/
bool beginOverrideStream() {
  ovStreamUp = false;
  if (!wifiUp || !fbReady || !Firebase.ready()) return false;
  String path = "/meters/" + deviceID + "/relayOverrides";
...
void loop() {
  ...
  if (wifiUp && fbReady) {
    if (!ovStreamUp) {
      // Not established. Retry on the slow interval rather than every pass:
      // the stream is an optimisation, and overrides still arrive on the
      // ordinary FB_PULL_MS poll while it is down, so failing to open it is
      // not worth spending the loop on.
      if (now - tOvRetry >= OVSTREAM_RETRY_MS) {
        tOvRetry = now;
        beginOverrideStream();
      }
    } else if (now - tOvStream >= OVSTREAM_CHECK_MS) {
      tOvStream = now;
      if (Firebase.RTDB.readStream(&ovStream)) {
        if (ovStream.streamAvailable()) {
          Serial.println("[STREAM] change: path=" + ovStream.dataPath() +
                         " type=" + ovStream.dataType());
          pullOverridesNow();
        }
      } else {
        // Report the first failure of an episode and then stay quiet: the
        // stream is dropped here, so the branch above owns the reconnect and
        // nothing is gained by restating the same reason every pass.
        if (!ovErrLogged) {
          Serial.println("[STREAM] read error: " + ovStream.errorReason() +
                         " -- dropping stream, will retry");
          ovErrLogged = true;
        }
        ovStreamUp = false;
        tOvRetry   = now;
      ...
      if (ovStream.streamTimeout()) {
        // Expected: Firebase closes a stream periodically server-side.
