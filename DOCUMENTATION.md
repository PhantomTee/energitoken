...
Access control on all of this is enforced by Firebase's security rules, which I deploy from `firebase/database.rules.json`. Every raw meter reading (voltage, power, relay state, and so on) has `.write: false` for any normal client, because the meter writes those fields using a database secret that bypasses the rules entirely, not through ordinary authenticated access. Only `budgetWh` and manual relay overrides are writable by the household itself.
...
Device pairing and unpairing also live here, since letting a phone write directly to Firebase's device-binding paths would mean anyone could claim someone else's meter. And as of the latest pass, there's a session-binding endpoint that verifies a wallet's own signature before Firebase is allowed to trust that a given anonymous session belongs to that wallet, which is worth its own explanation below.
...
I haven't flashed this onto a physical board and watched it run end to end yet. The three prototype sketches I built earlier established the real wiring (which pins, which sensor, which relay module) and this firmware builds on that, but a genuine bench test is still ahead of me.
...
Everything above the hardware line is built, deployed, and tested to the extent automated tests can cover it: the contract has sixteen passing Hardhat tests including the direct-bypass case, the app typechecks cleanly, and the Firebase rules and the latest contract are both live on their respective networks.

What's genuinely not done yet is the physical proof that all of this works together outside a simulator: flashing the firmware onto real hardware, wiring up all four relay channels (only the first one has been bench-tested so far), and walking through pairing, live budget enforcement, and a transfer with a real meter reporting real numbers. Multi-language support beyond English is deliberately deferred. And while I've reasoned carefully through the security fixes above and backed the contract-level one with tests, I haven't yet had a second person independently try to break any of it, which is worth doing before I'd call the security story finished rather than just improved.
